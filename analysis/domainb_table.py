...
def aggregate_domainb(run_dir: Path) -> Tuple[str, Dict[str, Any]]:
    ...
    for v in variants:
        ...
        # Only count replanning events (exclude phase accounting events).
        evs = [e for e in evs if str(e.get("event_type", "replan")) != "phase"]
        ...
        timeout_rate = (
            sum(
                1.0
                for e in eps
                if str(e.get("error", "") or "").strip().upper().startswith("TIME_BUDGET_EXCEEDED")
            )
            / n_eps
            if n_eps
            else float("nan")
        ...
        tok_after = [
            x for x in (_to_float(e.get("tokens_after_prune")) for e in evs) if x is not None and x > 0
        ]
    ...
    for r in rows:
        md.append(
            "| {variant} | {n} | {succ} | {dl} | {to} | {wt} | {ti} | {ta} | {tr} | {tap95} | {tap99} | {lp50} | {lp95} | {lp99} | {slo} | {sv} |\n".format(
                variant=r["variant"],
                n=r["episodes"],
                succ=_pct(float(r["success"])),
                dl=_pct(float(r["deadlock_rate"])),
                to=_pct(float(r["timeout_rate"])),
...
def main() -> None:
    ...
    md, payload = aggregate_domainb(run_dir)
